...
# ============================================================
# VENTAS MANUALES: BASE CANÓNICA (17/09/2026)
# ============================================================
...
#
# Ahora hay UNA sola base canónica para leer y escribir, y la base activa deja de
# importar. Se elige así:
#   1. `VENTAS_MANUALES_BASE` (variable de entorno), si está definida y válida.
#   2. `BASE_DEFAULT` de la instalación, si el usuario puede usarla.
#   3. La primera base permitida del usuario que exista en el catálogo.
# El punto 3 es una red de seguridad para no romper la carga cuando alguien no
# alcanza la canónica (queda registrado en el log). Si la instalación es
# homogénea —todos con acceso a BASE_DEFAULT—, siempre se usa la misma.

def base_ventas_manuales():
    """Nombre de la base donde vive VENTAS_MANUALES (ver bloque de arriba)."""
    from config import BASE_DEFAULT as _BASE_DEFAULT

    elegida = (os.environ.get('VENTAS_MANUALES_BASE') or '').strip()
    if elegida and elegida in BASES_DISPONIBLES:
        return elegida
    if elegida:
        logger.warning(f'VENTAS_MANUALES_BASE="{elegida}" no está en el catálogo; se ignora')

    # Bases permitidas del usuario de la sesión, si hay sesión.
    permitidas = None
    try:
        from modules.shared.decorators import bases_permitidas_actuales
        permitidas = bases_permitidas_actuales()      # None = todas
    except Exception:
        permitidas = None

    if permitidas is None or _BASE_DEFAULT in permitidas:
        return _BASE_DEFAULT
    for base in permitidas:
        if base in BASES_DISPONIBLES:
            logger.warning(
                'Ventas manuales: este usuario no alcanza la base canónica (%s); '
                'se usa %s. Las ventas cargadas podrían no verse desde otras PCs.'
                % (_BASE_DEFAULT, base))
            return base
    logger.error('Ventas manuales: no hay ninguna base utilizable; se usa %s' % _BASE_DEFAULT)
    return _BASE_DEFAULT


def get_db_connection_ventas_manuales():
    """Conexión a la base CANÓNICA de ventas manuales (ignora la base activa)."""
    base = base_ventas_manuales()
