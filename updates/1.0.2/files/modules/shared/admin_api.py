...
# ============================================================
# 🩺 DIAGNÓSTICO (vista nueva, 16/09/2026)
# ============================================================
# Responde "¿por qué no anda / no actualiza?". Solo LECTURA: no toca negocio ni
# configuración. Se apoya en `modules/shared/diagnostico.py`, que sí prueba el
# SQL de verdad (SELECT 1 por base, en paralelo y con timeout corto) y lee
# `logs/app.log`. Nunca devuelve cadenas de conexión ni credenciales.

# El informe completo prueba el SQL de cada base, así que se cachea unos segundos:
# entrar y salir de la vista no dispara 11 conexiones nuevas cada vez.
_DIAG_CACHE = {'momento': 0.0, 'datos': None}
_DIAG_CACHE_SEG = 45


@admin_bp.route('/diagnostico', methods=['GET'])
@requiere_autenticacion
@requiere_permiso(PermisosSistema.ADMIN_DIAGNOSTICO)
def diagnostico_sistema():
    """Chequeos en vivo + errores recientes del log."""
    try:
        import time as _time
        from . import diagnostico as diag
        from .decorators import bases_permitidas_actuales
        from .database import BASES_DISPONIBLES

        ahora = _time.time()
        if _DIAG_CACHE['datos'] and (ahora - _DIAG_CACHE['momento']) < _DIAG_CACHE_SEG:
            cacheado = dict(_DIAG_CACHE['datos'])
            cacheado['cacheado'] = True
            return jsonify(cacheado)

        permitidas = bases_permitidas_actuales()
        # None = todas (superadmin o comodín '*'); [] = ninguna permitida.
        bases = list(BASES_DISPONIBLES if permitidas is None else permitidas)

        datos = diag.informe(bases=bases)
        datos['bases_consultadas'] = bases
        datos['cacheado'] = False
        _DIAG_CACHE['datos'] = datos
        _DIAG_CACHE['momento'] = ahora
        return jsonify(datos)
    except Exception as e:
        logger.error(f'Error en diagnostico_sistema: {e}')
        return jsonify({"error": f"Error inesperado: {str(e)}"}), 500


@admin_bp.route('/diagnostico/errores', methods=['GET'])
@requiere_autenticacion
@requiere_permiso(PermisosSistema.ADMIN_DIAGNOSTICO)
def diagnostico_errores():
    """Solo los errores agrupados del log (sin sondear el SQL: es inmediato)."""
    try:
        from . import diagnostico as diag
        ...
        if horas <= 0 or horas > 720:
            horas = 24
        entradas = diag.leer_lineas_log()
        return jsonify({
            'errores': diag.errores_agrupados(entradas, horas=horas),
            'lineas_leidas': len(entradas),
            'horas': horas,
        })
    except Exception as e:
        logger.error(f'Error en diagnostico_errores: {e}')
        return jsonify({"error": f"Error inesperado: {str(e)}"}), 500


@admin_bp.route('/diagnostico/alerta', methods=['GET'])
@requiere_autenticacion
@requiere_permiso(PermisosSistema.ADMIN_DIAGNOSTICO)
def diagnostico_alerta():
    """Insignia del menú lateral: contador liviano de problemas nuevos.

    Solo lee el log (no sondea SQL ni el canal de actualizaciones), porque el
    panel lo consulta cada pocos minutos. `desde` es la marca de la última vez
    que esa PC abrió la pestaña, para contar únicamente lo nuevo.
    """
    try:
        from . import diagnostico as diag

        horas = request.args.get('horas', default=72, type=int)
        if horas <= 0 or horas > 720:
            horas = 72
        return jsonify(diag.resumen_alerta(horas=horas, desde=request.args.get('desde')))
    except Exception as e:
        logger.error(f'Error en diagnostico_alerta: {e}')
        return jsonify({"error": f"Error inesperado: {str(e)}"}), 500
