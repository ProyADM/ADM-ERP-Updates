...
function renderVistaUsuariosAdmin() {
  ...
  body.innerHTML =
    '<div class="admin-toolbar">' +
      '<input type="text" id="admin-buscar-usuarios" data-tooltip="adminUsuariosBuscar" placeholder="Buscar por usuario, nombre o email…">' +
...
function _htmlChecksPermisos(idMulti, seleccionados) {
  var clave = (idMulti === 'adminUsuariosRestringidos')
    ? 'adminUsuarioPermisosRestringidos' : 'adminUsuarioPermisosExtra';
  // Mismo helper de grupos plegables que el editor de roles: el estado
  // abierto/cerrado se recuerda por grupo en el navegador.
  return Object.keys(_admPermisos).sort().map(function (grupo) {
    return adminGrupoPlegable(grupo, _admPermisos[grupo], seleccionados, idMulti, clave);
  }).join('');
}
...
function _htmlCamposEdicionUsuarioAdmin(base) {
  ...
  var opcionesBases = _admPuedeEditarBases
    ? adminMulti('adminUsuariosBases',
        ...
        }), basesActuales, { comodin: 'Todas las bases', tooltip: 'adminUsuarioBases' })
  ...
  var opcionesModulos = _admPuedeEditarModulos
    ? adminMulti('adminUsuariosModulos',
        ...
        }), modulosActuales, { comodin: 'Todos los módulos', tooltip: 'adminUsuarioModulos' })
    : _htmlEditorSoloLectura('admin.modulos.config', 'los módulos', modulosActuales, 'Todos');
  // Las dos listas van envueltas en `.admin-checks`: sin ese contenedor, la
  // grilla no se aplicaba y las bases y los módulos quedaban apilados en UNA
  // sola columna (la versión de solo lectura sí las envolvía).
  // El ⓘ va en la ETIQUETA del campo (data-tooltip en el <label>): en las
  // casillas la ayuda solo se ve al pasar el mouse, así que sin ícono visible
  // nadie se entera de que hay ayuda. `adminReaplicarPermisos` lo engancha al
  // cerrar el render.
  // Base por defecto del usuario (con la que arranca la app). Es una PREFERENCIA
  // de arranque, no un permiso: se ofrecen solo las bases que tiene permitidas.
  var baseDefault = base.base_default || '';
  var basesParaDefault = _admBases.filter(function (b) {
    return basesActuales.indexOf('*') >= 0 || basesActuales.indexOf(b.id) >= 0;
  });
  var opcionesBaseDefault = '<option value="">— la primera permitida —</option>' +
    basesParaDefault.map(function (b) {
      return '<option value="' + escapeHTML(b.id) + '"' +
             (b.id === baseDefault ? ' selected' : '') + '>' +
             escapeHTML(b.nombre + ' (' + (b.pais || b.id) + ')') + '</option>';
    }).join('');
  return '<div class="admin-campo"><label data-tooltip="adminUsuarioBases">Bases permitidas</label>' +
      '<div class="admin-checks">' + opcionesBases + '</div></div>' +
    '<div class="admin-campo"><label data-tooltip="adminUsuarioBaseDefault">Base por defecto (con la que entra)</label>' +
      '<select id="admin-usuario-base-default">' + opcionesBaseDefault + '</select></div>' +
    '<div class="admin-campo"><label data-tooltip="adminUsuarioModulos">Módulos permitidos</label>' +
      '<div class="admin-checks">' + opcionesModulos + '</div></div>' +
    '<div class="admin-campo"><label data-tooltip="adminUsuarioPermisosExtra">Permisos extra (se suman a los del rol)</label>' +
      (_admPuedeEditarPermisos
    ...
    '<div class="admin-campo"><label data-tooltip="adminUsuarioPermisosRestringidos">Permisos restringidos (se restan)</label>' +
...
function htmlFormUsuarioAdmin(base, editando) {
  var opcionesRoles = _admPuedeEditarRoles
    ? '<div class="admin-checks">' + adminMulti('adminUsuariosRoles', _admRoles.map(function (r) {
      ...
      }), base.roles || [], { tooltip: 'adminUsuarioRoles' }) + '</div>'
  ...
  return '' +
    '<div class="admin-panel">' +
      ...
      (editando ? '' :
        ...
        '<input type="text" id="admin-usuario-username" data-tooltip="adminUsuarioUsername" value="' + escapeHTML(base.username || '') + '"></div>') +
      '<div class="admin-campo"><label>Nombre</label>' +
        '<input type="text" id="admin-usuario-nombre" data-tooltip="adminUsuarioNombre" value="' + escapeHTML(base.nombre || '') + '"></div>' +
      '<div class="admin-campo"><label>Email</label>' +
        '<input type="text" id="admin-usuario-email" data-tooltip="adminUsuarioEmail" value="' + escapeHTML(base.email || '') + '"></div>' +
      '<div class="admin-campo"><label data-tooltip="adminUsuarioRoles">Roles</label>' + opcionesRoles + '</div>' +
      (editando ? _htmlCamposEdicionUsuarioAdmin(base) : '') +
      '<div class="admin-toolbar" style="margin-top:14px">' +
        adminBotonAccion('Guardar', 'guardar', null, 'admin-btn-primario', null, 'adminUsuarioGuardar') +
...
async function abrirFormUsuarioAdmin(username) {
  ...
  // Plegado de los grupos de permisos: el encabezado abre/cierra, salvo que el
  // clic venga del botón "tildar/destildar todo" (que tiene que hacer solo eso).
  adminEngancharPlegado('admin-usuario-form');
...
async function guardarUsuarioAdmin() {
  ...
  try {
    ...
    if (_admEditandoUsuario) {
      ...
      // Base por defecto: se manda SOLO si el selector está y hay catálogo de
      // bases (misma regla que el resto: sin el editor pintado se omite para no
      // borrar lo que el usuario ya tenía).
      var selBaseDefault = document.getElementById('admin-usuario-base-default');
      if (_admPuedeEditarBases && selBaseDefault) cuerpo.base_default = selBaseDefault.value || null;
