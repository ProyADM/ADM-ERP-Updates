...
def _texto_de_error_propio(e, generico, permitir_error_fk=False):
    ...
    Los lotes del módulo de stock marcan sus errores de negocio con
    RAISERROR('STK_...|mensaje'): ese texto lo escribió la app (no expone
    estructura de la base) y tiene que llegar al endpoint para poder responder
    400/409 con el código. Por eso se deja pasar SOLO si trae el prefijo 'STK_';
    cualquier otro error de la base sigue saliendo con el mensaje genérico de
    siempre, para no filtrar constraints ni internos a otros módulos.

    `permitir_error_fk=True` (lo usa SOLO el camino de stock, con
    `run_sql(..., permitir_error_fk=True)`) deja pasar además el texto de un 547
    (violación de FK) para que el endpoint de stock lo traduzca con
    `mensaje_de_error_sql` a "está referenciado por <circuito>". Sin este pedido
    explícito no sale: los demás módulos muestran `str(e)` al cliente y no tienen
    por qué ver el nombre de la constraint.
    """
    texto = str(e)
    inicio = texto.find('STK_')
    if inicio < 0:
        inicio = _inicio_error_fk(texto) if permitir_error_fk else -1
    if inicio < 0:
        return generico
    # Se recorta el envoltorio de pyodbc ("('42000', '[42000] ... [SQL Server]")
    # y las comillas/paréntesis de cierre, así mensaje_de_error_sql() parsea el
    # código y el mensaje limpios.
