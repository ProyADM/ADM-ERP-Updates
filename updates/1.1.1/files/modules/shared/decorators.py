...
def bases_permitidas_actuales():
    """Bases que el usuario de la sesión puede tocar, o None si son todas.

    Es lo que se le pasa a los servicios que recorren varias bases para que
    consulten SOLO las autorizadas: antes iteraban `BASES_DISPONIBLES` completo
    y filtraban información de países que el usuario no tenía permitidos.
    """
    username = session.get('username')
    if not username:
        return []
    datos = gestor.datos_usuario_completo(username) or {}
    if datos.get('es_superadmin'):
        return None
    bases = datos.get('bases_permitidas') or []
    if '*' in bases:
        return None
    return bases


def chequear_acceso_alguna_base():
    """Valida que el usuario tenga AL MENOS UNA base permitida.

    Para reportes multi-base que ya respetan `bases_permitidas`: el consolidado
    puede ser parcial y la respuesta dice sobre qué bases se calculó. Reemplaza a
    `chequear_acceso_total_bases` en Ventas Globales (14/09/2026): antes exigía
    `'*'`, así que un usuario con 2 bases no podía abrir el reporte — pero sí
    podía ver las 11 bases por Ventas por País, que no validaba nada.
    """
    username = session.get('username')
    if not username:
        return jsonify({
    ...
    # OJO con la diferencia: None = todas las bases (permitido), [] = ninguna.
    # Un `if not bases` trataría a None como "sin acceso" y dejaría al superadmin
    # afuera de su propio reporte.
    bases = bases_permitidas_actuales()
    if bases is not None and len(bases) == 0:
        logger.warning(f"Reporte multi-base sin ninguna base habilitada: {username}")
        return jsonify({
            "error": "No tenés ninguna base habilitada para consultar este reporte",
            "code": "BASE_FORBIDDEN"
        }), 403
    return None