...
def _bases_a_consultar(base_override, bases_permitidas):
    """Bases que este pedido puede tocar (fail-closed).

    `bases_permitidas` viene del usuario autenticado; None o ['*'] = todas.
    Antes se iteraba `BASES_DISPONIBLES` completo sin mirar el permiso, y como
    este endpoint (Ventas por País) sólo exige `reportes.ver`, un usuario con
    2 bases recibía las ventas de los 11 países.
    """
    if base_override:
        if base_override not in BASES_DISPONIBLES:
            return {}
        return {base_override: BASES_DISPONIBLES[base_override]}
    if bases_permitidas is None or '*' in bases_permitidas:
        return BASES_DISPONIBLES
    return {b: BASES_DISPONIBLES[b] for b in bases_permitidas if b in BASES_DISPONIBLES}
...
def obtener_consolidado_pais(anio, base_override=None, sociedad_override=None,
    ...
    bases_fallidas = []
    try:
        bases_a_consultar = _bases_a_consultar(base_override, bases_permitidas)
        ...
        for db_name, db_info in bases_a_consultar.items():
            try:
                ...
                # No se pierde en silencio: si una base permitida no responde, el
                # reporte sale igual pero AVISA. Antes sólo quedaba un warning en
                # el log y el usuario veía un consolidado incompleto sin saberlo.
        ...
        return {
            'success': True,
            'data': detalle,
            # Trazabilidad para el frontend: qué bases entraron y cuáles fallaron.
            # El rotulado del total depende de esto (un consolidado de 2 de 11
            # bases NO es el total de la empresa).
            'bases_incluidas': sorted(bases_a_consultar.keys()),
            'bases_fallidas': sorted(bases_fallidas),
