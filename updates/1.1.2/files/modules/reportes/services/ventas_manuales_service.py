...
from .utils import (get_db_connection, get_db_connection_base, get_db_connection_ventas_manuales,
                    asegurar_tabla_ventas_manuales, obtener_cotizacion_por_mes,
                    ErrorBaseVentasManuales)
...
def listar_ventas_manuales(base=None, anio=None, mes=None):
    ...
    try:
        ...
        # Base canónica: la lista se arma igual sin importar la base activa.
...
def crear_venta_manual(base, sociedad, anio, mes, pais, importe_usd, importe_ps, fecha_registro, comentario='', usuario='anonimo', centro_costo=None):
    ...
    """
    try:
        asegurar_tabla_ventas_manuales(para_escritura=True)
        ...
        # Base CANÓNICA de ventas manuales: la venta se guarda ahí sin importar
        # la base activa del usuario, así se puede cargar cualquier país desde
        # cualquier base (17/09/2026).
        conn, _ = get_db_connection_ventas_manuales(para_escritura=True)
        cursor = conn.cursor()
        insertados = 0
        errores = []
...
def obtener_venta_manual(venta_id, para_escritura=False):
    """Una venta manual por id (o None). Se usa para autorizar el borrado.

    El borrado la llama con `para_escritura=True`: si el usuario no alcanza la
    base canónica, el error de permiso (-el mensaje que aprobó el usuario-) se
    propaga en vez de convertirse en un `None` que la ruta traduce a "La venta no
    existe". Cualquier otro error sigue devolviendo `None` como antes.
    """
    try:
        asegurar_tabla_ventas_manuales(para_escritura=para_escritura)
        conn, _ = get_db_connection_ventas_manuales(para_escritura=para_escritura)
        cursor = conn.cursor()
        cursor.execute("SELECT id, base, sociedad, anio, mes, pais, importe_usd, importe_ps, "
                       "usuario FROM VENTAS_MANUALES WHERE id = ?", (venta_id,))
        fila = cursor.fetchone()
        conn.close()
        if not fila:
            return None
        return {'id': fila[0], 'base': fila[1], 'sociedad': fila[2], 'anio': fila[3],
                'mes': fila[4], 'pais': fila[5], 'importe_usd': fila[6],
                'importe_ps': fila[7], 'usuario': fila[8]}
    except ErrorBaseVentasManuales:
        # Sin acceso a la canónica: no es "no existe", es "no podés". Se propaga.
        raise
    except Exception as e:
        logger.error(f"Error obteniendo venta manual {venta_id}: {e}")
        return None
...
def eliminar_venta_manual(venta_id):
    ...
    try:
        asegurar_tabla_ventas_manuales(para_escritura=True)
        # Base canónica: se borra de donde realmente vive la fila.
        conn, _ = get_db_connection_ventas_manuales(para_escritura=True)
        cursor = conn.cursor()
...
def importar_ventas_manuales(df, usuario='anonimo'):
    ...
    try:
        ...
        # Una conexión única a la base CANÓNICA para todos los inserts
        # (la importación puede traer filas de varios países).
        conn, _ = get_db_connection_ventas_manuales(para_escritura=True)
        cursor = conn.cursor()
