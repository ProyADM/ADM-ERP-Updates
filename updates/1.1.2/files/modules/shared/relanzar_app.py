...
def limpiar_estado(install_dir, log_fn):
    ...
    for root, dirs, files in os.walk(install_dir):
        for d in list(dirs):
            if d in ('__pycache__', 'flask_session'):
    ...
    if total:
        log_fn(f'Limpiado: {total} carpeta(s) de cache/sesion')
    else:
        log_fn('Nada que limpiar (sin __pycache__ ni flask_session)')
    ...
    while time.time() - t0 < ESPERA_ARRANQUE_SEG:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if s.connect_ex(('127.0.0.1', puerto)) == 0:
                ...
                limpiar_estado(install_dir, log_fn)
