...
def calcular_diff(manifest_nuevo, manifest_anterior):
    ...
    # 🔴 DESACTIVADO: No eliminar archivos automáticamente
    archivos_eliminados = []  # Siempre vacío
    # archivos_eliminados = [rel_path for rel_path in manifest_anterior if rel_path not in manifest_nuevo]
...
def guardar_deleted_json(version, archivos_eliminados):
    ...
    🔴 Esta función se mantiene pero solo se llama si archivos_eliminados no está vacío.
    """
...
def generar_version_json(version, changelog, hay_deleted):
    """
    Genera el archivo version.json para el servidor.
    """
    version_info = {
        'version': version,
...
if __name__ == "__main__":
    ...
    # 6. Registrar archivos eliminados (solo si hay)
