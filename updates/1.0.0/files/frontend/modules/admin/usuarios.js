...
async function cargarVistaUsuariosAdmin() {
  ...
  _admPermisos = (rP.ok && rP.datos) || {};      // sin admin.permisos.asignar queda {}
...
function _admOpcionesComodin(items, actuales, etiquetaComodin) {
  var opciones = items.slice();
  if ((actuales || []).indexOf('*') >= 0) {
    opciones.unshift({ valor: '*', etiqueta: etiquetaComodin });
  }
  return opciones;
}

// Valores actuales en solo lectura, para cuando el editor no se pudo cargar.
...
function _htmlCamposEdicionUsuarioAdmin(base) {
  ...
  var opcionesBases = _admPuedeEditarBases
    ? adminMulti('adminUsuariosBases',
        _admOpcionesComodin(_admBases.map(function (b) {
          return { valor: b.id, etiqueta: b.nombre + ' (' + (b.pais || b.id) + ')' };
        }), basesActuales, 'Todas'), basesActuales)
  ...
  var opcionesModulos = _admPuedeEditarModulos
    ? adminMulti('adminUsuariosModulos',
        _admOpcionesComodin(_admModulos.map(function (m) {
          return { valor: m.id, etiqueta: m.nombre };
        }), modulosActuales, 'Todos'), modulosActuales)
