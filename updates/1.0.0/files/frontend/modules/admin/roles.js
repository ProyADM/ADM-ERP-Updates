...
async function cargarVistaRolesAdmin() {
  ...
  _admPermisosGrupos = (rP.ok && rP.datos) || {};   // sin admin.permisos.asignar queda {}
...
function renderVistaRolesAdmin() {
  ...
  body.innerHTML =
    '<div class="admin-toolbar">' +
      adminBoton('+ Nuevo rol', "abrirFormNuevoRolAdmin()", 'admin-btn-primario', 'admin.roles.crear') +
  ...
  cont.innerHTML =
    '<div class="admin-panel"><h4>Nuevo rol</h4>' +
      '<div class="admin-campo"><label>Identificador (sin espacios)</label>' +
        '<input type="text" id="admin-rol-id" placeholder="deposito"></div>' +
      '<div class="admin-campo"><label>Nombre</label>' +
        '<input type="text" id="admin-rol-nombre" placeholder="Depósito"></div>' +
      '<div class="admin-campo"><label>Descripción</label>' +
        '<input type="text" id="admin-rol-descripcion"></div>' +
      '<div class="admin-toolbar" style="margin-top:12px">' +
        adminBotonAccion('Crear', 'crear', null, 'admin-btn-primario') +
  ...
  if (puedeGuardar) {
    cuerpoPermisos = Object.keys(_admPermisosGrupos).sort().map(function (grupo) {
      return '<div class="admin-grupo">' +
        '<div class="admin-grupo-titulo">' + escapeHTML(grupo) + ' ' +
          adminBotonAccion('tildar/destildar todo', 'tildar-grupo', grupo, '') +
        '</div><div class="admin-checks">' +
        adminMulti('adminRolPermisos', _admPermisosGrupos[grupo], permisosActuales) +
        '</div></div>';
    }).join('');
  ...
  cont.innerHTML =
    '<div class="admin-panel"><h4>' + (puedeGuardar ? 'Permisos del rol ' : 'Rol ') +
      ...
      '<div class="admin-campo"><label>' +
        ...
        '</label>' + cuerpoPermisos + '</div>' +
      '<div class="admin-toolbar" style="margin-top:12px">' +
        (puedeGuardar ? adminBotonAccion('Guardar', 'guardar', null, 'admin-btn-primario') : '') +
...
function tildarGrupoRolAdmin(grupo) {
  ...
  var permisos = _admPermisosGrupos[grupo] || [];
