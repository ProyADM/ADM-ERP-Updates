...
// Además dice cómo y cuándo se aplicó la última (`ultimo_estado` del diagnóstico:
// la actualización es automática al arrancar, así que "a mano" es la excepción).
function _admDiagBloqueVersion(canal) {
  ...
  var ultimo = canal.ultimo_estado || {};
  var detalle = '';
  if (ultimo.aplicada_en) {
    var como = ultimo.origen === 'automatica' ? 'automática al inicio' : 'manual';
    detalle = ' · última: ' + como + ' (' + ultimo.aplicada_en + ')';
  }
  if (ultimo.estado === 'error') {
    chip = 'warn';
    texto += ' · la última actualización falló' + (ultimo.error ? ' (' + ultimo.error + ')' : '');
  }
  return '<div class="admin-dato"><b>Versión</b><span>' +
         _admDiagChip(chip, texto + detalle) + '</span></div>';
}
...
function renderVistaDiagnosticoAdmin() {
  ...
  var filas = bases.map(function (b) {
    ...
    // Columna "Partidas": numerador del ERP (`SIST_NUSI` id 6) contra el máximo
    // real de `STOC_PART` de esa base, que es el chequeo de sólo lectura que
    // agregó `modules/shared/diagnostico.py` (`sql.bases[].partidas`). Se muestra
    // el par contador/máximo cuando se pudo leer y el aviso cuando el contador
    // quedó ATRÁS: en ese caso la próxima alta de partida del ERP choca con
    // `PART_PK1` (error 2627, el incidente del 21/09/2026 en la base AR).
    var par = b.partidas || {};
    var parChip, parTexto;
    if (par.contador === null || par.contador === undefined
        || par.maximo === null || par.maximo === undefined) {
      // Sin los dos valores no se afirma nada: gris si no hay aviso (la base no
      // respondió, o el chequeo no aplica) y aviso si el chequeo no pudo leer.
      parChip = par.aviso ? 'warn' : 'gris';
      parTexto = par.aviso ? 'sin dato' : '—';
    } else if (par.aviso) {
      parChip = 'warn';
      parTexto = par.contador + ' < ' + par.maximo;
    } else {
      parChip = 'ok';
      parTexto = par.contador + ' / ' + par.maximo;
    }
    return [
      ...
      detalle,
      { html: _admDiagChip(parChip, parTexto) }
    ];
  });
  ...
  var bloque1 =
    '<div class="admin-panel"><h4>Estado actual</h4>' +
      ...
      '</div>' +
      (bases.length
        ? adminTabla(['Base', 'Servidor', 'Respuesta', 'Detalle', 'Partidas'], filas)
        : '<div class="admin-aviso">Tu usuario no tiene bases habilitadas para comprobar.</div>') +
      (bases.length
        ? '<div class="admin-check-pie" style="margin:-4px 0 8px">Partidas: numerador de partidas del ERP'
          + ' (SIST_NUSI id 6) contra el máximo real de STOC_PART. Si el numerador queda atrás (se muestra'
          + ' como 1486 &lt; 1487), la próxima alta de partida del ERP choca con PART_PK1 (error 2627).</div>'
        : '') +
