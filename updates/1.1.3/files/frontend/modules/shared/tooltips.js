...
// Datos de la sesión para los tooltips dinámicos (el indicador de usuario de la
// barra es SÓLO un ícono: todo lo que se sabe de la sesión se lee en su tooltip).
// Devuelve null si todavía no hay usuario cargado: ahí no se puede afirmar nada.
function _sesionUsuario() {
    var u = null;
    try { u = JSON.parse(localStorage.getItem('user') || 'null'); } catch (e) { u = null; }
    if (!u || (!u.nombre && !u.username)) return null;
    var rol;
    if (u.es_superadmin === true || u.rol === 'superadmin') {
        rol = 'SuperAdmin';
    } else if (u.rol) {
        var texto = String(u.rol);
        rol = texto.charAt(0).toUpperCase() + texto.slice(1);
    } else {
        rol = 'sin rol';
    }
    return {
...
export const TOOLTIPS = {
    ...
    'totalFacturaInput': {
        ...
        descripcion: 'Monto total de la factura: incluye el IVA y, cuando la factura lo trae, también el impuesto especial (IDP del combustible o impuesto al turismo).',
        accion: 'Si la factura trae impuesto especial, cargalo en el campo "Impuesto especial": el bruto y el IVA no salen solos del total, hay que restarle ese impuesto.'
    },
    'especialFacturaInput': {
        titulo: '🧾 Impuesto especial (IDP / turismo)',
        descripcion: 'No es IVA: es un impuesto especial que la factura trae dentro del total (el IDP del combustible o el impuesto al turismo del alojamiento). Se toma como un gasto más, así que queda incluido en el bruto y se resta del total antes de calcular el 12%.',
        accion: 'Dejalo vacío o en 0 cuando la factura no traiga impuesto especial. En FCC (pequeño contribuyente) no se usa: ahí el total ya es el bruto y no hay nada que separar.'
    },
    'brutoFacturaInput': {
        titulo: '💰 Bruto (sin IVA)',
        descripcion: 'Monto del gasto antes del IVA. Si la factura trae impuesto especial, el bruto es la base imponible MÁS ese impuesto (es un gasto de la factura).',
        accion: 'La base imponible es el bruto menos el impuesto especial, y sobre ella se calcula el IVA (12%). La fila de impuestos del ERP (`CPAG_ICTP`) lleva esa BASE en `ICTP_IMP_GRA_ORI` y el 12% en `ICTP_IMPUESTO_ORI`. Se calcula automáticamente y se valida con la suma de los renglones.'
    },
    'ivaFacturaInput': {
        titulo: '💰 IVA',
        descripcion: 'Impuesto al Valor Agregado: 12% para FCP y 0% para FCC.',
        accion: 'Es el 12% de la base imponible, y la base es el total menos el impuesto especial, dividido 1,12 (con total 150,00 y especial 5,26: base 129,23 e IVA 15,51). En FCC es 0 porque el total ya es el bruto.'
    },
    ...
    'barraUsuario': {
        // Los DOS campos son funciones: en la barra sólo está el ícono 👤, así que
        // el nombre, el usuario y el rol se leen acá (e `init.js` copia el mismo
        // texto al `aria-label`). `tooltip-component.js` escapa lo que devuelve una
        // función, porque sale de la sesión (dato del servidor).
        // CONVENCIÓN del catálogo: los campos con datos de la sesión van como
        // FUNCIÓN (así se escapan); los de texto son del código y se interpolan
        // tal cual. No metas datos dinámicos en un string.
        titulo: function () {
            var s = _sesionUsuario();
            return s ? '👤 ' + s.nombre : '👤 Usuario';
        },
        descripcion: function () {
            var s = _sesionUsuario();
            if (!s) return 'Sin sesión iniciada (la sesión todavía no se cargó).';
            return 'Usuario: ' + (s.usuario || s.nombre) + ' — Rol: ' + s.rol;
        },
    ...
    'barraVersion': {
        titulo: '🏷️ Versión instalada',
        descripcion: 'Qué versión del ERP está corriendo esta PC.',
        accion: 'Sirve para reportar un problema diciendo la versión. El detalle del estado de actualización está en Administración → Diagnóstico.'
    },
    'botonActualizar': {
        titulo: '🔄 Actualizar',
