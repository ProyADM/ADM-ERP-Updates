...
# Prefijos de los errores que escribe la APP (no la base): su texto no expone
# estructura del motor y tiene que llegar al endpoint para responder con codigo.
PREFIJOS_ERROR_PROPIO = ('STK_', 'CXP_')


def _inicio_error_propio(texto):
    """Posicion del primer error propio de la app ('STK_' o 'CXP_') o -1.

    Se usa el que aparezca PRIMERO: un lote de un modulo no deberia traer el
    prefijo del otro, pero si el envoltorio de pyodbc mencionara a los dos, el
    codigo que importa es el del error original.
    """
    inicio = -1
    for prefijo in PREFIJOS_ERROR_PROPIO:
        posicion = texto.find(prefijo)
        if posicion >= 0 and (inicio < 0 or posicion < inicio):
            inicio = posicion
    return inicio


def _texto_de_error_propio(e, generico, permitir_error_fk=False):
    """Mensaje con el que se levanta el RuntimeError de run_sql.

    Los lotes de los modulos endurecidos marcan sus errores de negocio con
    RAISERROR('STK_...|mensaje') y RAISERROR('CXP_...|mensaje'): ese texto lo
    escribio la app (no expone estructura de la base) y tiene que llegar al
    endpoint para poder responder 400/409 con el codigo. Por eso se deja pasar
    SOLO si trae uno de esos prefijos; cualquier otro error de la base sigue
    saliendo con el mensaje generico de siempre.

    `permitir_error_fk=True` (lo usan SOLO los caminos de stock y de CxP, con
    `run_sql(..., permitir_error_fk=True)`) deja pasar ademas el texto de un 547
    (violacion de FK) para que el endpoint lo traduzca a "esta referenciado por
    <circuito>". Sin este pedido explicito no sale: los demas modulos muestran
    `str(e)` al cliente y no tienen por que ver el nombre de la constraint.
    """
    texto = str(e)
    inicio = _inicio_error_propio(texto)
    if inicio < 0:
        inicio = _inicio_error_fk(texto) if permitir_error_fk else -1
    if inicio < 0:
        return generico
    # Se recorta el envoltorio de pyodbc ("('42000', '[42000] ... [SQL Server]")
    # y las comillas/paréntesis de cierre, así mensaje_de_error_cxp() y
    # mensaje_de_error_sql() parsean el código y el mensaje limpios.
    return texto[inicio:].rstrip(" \t\r\n'\")")


def _inicio_error_fk(texto):
    """Posición donde empieza el mensaje de un 547 (violación de FK) o -1.

    'REFERENCE' aparece tanto en el mensaje en inglés ("conflicted with the
    REFERENCE constraint") como en el español ("restricción REFERENCE")."""
    mayusculas = texto.upper()
    for marca in ('CONFLICTED WITH THE REFERENCE', 'REFERENCE CONSTRAINT',
                  'RESTRICCIÓN REFERENCE', 'RESTRICCION REFERENCE'):
        posicion = mayusculas.find(marca)
        if posicion >= 0:
            return posicion
    return -1
...
def run_sql(query, params=(), fetch=True, server_override=None, database_override=None, username_override=None, password_override=None, timeout=30, permitir_error_fk=False):
    """
    ...
    Args:
        ...
        permitir_error_fk: True deja pasar el texto de un 547 (violación de FK)
            para que el endpoint que lo pidió lo traduzca (solo el de stock).
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(_texto_de_error_propio(e, "Error de conexión a la base de datos", permitir_error_fk))
        except pyodbc.IntegrityError as e:
            logger.error(f"[SQL ERROR de integridad] {database}: {e}")
            raise RuntimeError(_texto_de_error_propio(e, "Error de integridad de datos", permitir_error_fk))
        except pyodbc.ProgrammingError as e:
            logger.error(f"[SQL ERROR de sintaxis] {database}: {e}")
            raise RuntimeError(_texto_de_error_propio(e, "Error en la consulta SQL", permitir_error_fk))
        except pyodbc.Error as e:
            logger.error(f"[SQL ERROR] {database}: {e}")
            raise RuntimeError(_texto_de_error_propio(e, "Error ejecutando SQL", permitir_error_fk))
        except Exception as e:
            logger.error(f"[SQL ERROR inesperado] {database}: {e}")
            raise RuntimeError(_texto_de_error_propio(e, "Error inesperado en la base de datos", permitir_error_fk))
        finally:
            if conn:
                try:
                    conn.close()
