...
def listar_bases():
    # Import diferido: `modules.cxp` importa `.routes` (Flask + config) y este
    # endpoint se usa en el arranque de la app.
    from modules.cxp.servicio_comprobantes import PARAMETROS_CXP
    out = []
    for cod, info in BASES_DISPONIBLES.items():
        item = {"codigo": cod, "label": info["label"], "sigla": info["sigla"],
                # El frontend ya no tiene la constante de pais: el modulo sabe
                # para que bases esta habilitado (hoy solo Guatemala).
                "cxp_habilitado": cod in PARAMETROS_CXP}
        if "sociedades" in info:
            item["sociedades"] = [{"codigo": s, "label": d["label"]} for s, d in info["sociedades"].items()]
        out.append(item)
    return jsonify({
        "bases": out,
        # Base con la que arranca ESTE usuario (17/09/2026). Antes el frontend
        # forzaba 'plataforma_rd' para todos, así que cada usuario tenía que
        # cambiar de base a mano al entrar. `default` sigue siendo el del sistema
        # (compatibilidad); `base_default_usuario` es el preferido del usuario.
        "default": BASE_DEFAULT,
        "base_default_usuario": _base_default_de_sesion() or BASE_DEFAULT,
        "sociedad_default": SOCIEDAD_DEFAULT
    })


def _base_default_de_sesion():
    """Base por defecto del usuario de la sesión, o None.

    Solo se devuelve si es una base que el usuario PUEDE usar (fail-closed): un
    `base_default` de una base que ya no tiene permitida no debe hacer que la app
    arranque en una base prohibida. Si no hay preferencia, se usa la primera base
    permitida que exista en el catálogo.
    """
    try:
        ...
        if not username:
            return None
        datos = get_gestor_usuarios().datos_usuario_completo(username) or {}
        permitidas = datos.get('bases_permitidas') or []
        todas = datos.get('es_superadmin') or ('*' in permitidas)

        elegida = datos.get('base_default')
        if elegida and (todas or elegida in permitidas) and elegida in BASES_DISPONIBLES:
            return elegida

        if todas:
            return None                      # que decida el default del sistema
        for base in permitidas:
            if base in BASES_DISPONIBLES:
                return base
    except Exception as e:
        logger.warning(f'No se pudo resolver la base por defecto del usuario: {e}')
    return None
...
def reiniciar():
    ...
    El destino es la INSTALACION registrada (`APP_INSTALL_DIR`), que es donde el
    updater aplica los archivos: a este endpoint lo llama el boton "Actualizar
    ahora" del banner viejo, o sea "reiniciar para terminar de aplicar". En un
    cliente esa raiz es la misma desde la que corre la app; donde el registro
    apunta a otra carpeta (esta PC de desarrollo, que actualiza a la instalada)
    se reinicia la instalacion y no el arbol en ejecucion. Declarado como V12 en
    el brief del fix del relanzador. Ojo: `lanzar_relanzador` busca el SCRIPT en
    el arbol que corre (`app_dir`), que es otra cosa.
    """
    def _restart():
        time.sleep(0.5)
        try:
            # Raiz a REINICIAR (no es la raiz donde se busca el script).
            raiz_a_reiniciar = current_app.config.get('APP_INSTALL_DIR') or os.getcwd()
            puerto = int(os.environ.get('APP_PORT', '5000'))
            ok, detalle = lanzar_relanzador(raiz_a_reiniciar, puerto=puerto,
                                            motivo='api_reiniciar')
            if not ok:
