...
def importes_locales(bruto, iva, total, moneda, cotizacion):
    """Importes locales del comprobante. En quetzales (PS) no hay conversion: la
    cotizacion es 1 y el importe local es el del documento."""
    if moneda == 'PS':
...
def validar_alta(ctx, datos):
    ...
    locales = importes_locales(bruto, iva, total, moneda, cotizacion)
...
def verificar_iva(datos):
    """El IVA cargado tiene que coincidir con la tasa declarada, con la
    tolerancia del redondeo al centavo del documento (0,05)."""
    esperado = (datos['imp_bruto'] * datos['tasa_iva'] / Decimal(100)).quantize(
    ...
    if abs(datos['imp_iva'] - esperado) > TOLERANCIA_IVA:
        raise ErrorValidacion(
            ...
            str(datos['tasa_iva']) + "% del bruto (" + str(datos['imp_bruto']) +
...
INSERT INTO CPAG_ICTP (
    ...
    @ctacte, 'IVP', 'P12',
    {num(datos['imp_bruto'])}, {num(datos['imp_bruto_loc'])},
