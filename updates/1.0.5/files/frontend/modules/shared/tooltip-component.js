...
export function initTooltips() {
    ...
    elementos.forEach(el => {
        ...
        // Si el elemento es un botón/control, su propio texto ya hace de ancla:
        // no se le mete un ícono adentro (rompería el layout del botón). La ayuda
        // se muestra al pasar el mouse por el control.
        const esControl = el.matches('button, input, select, textarea, a');
...
function mostrarTooltip(key, targetElement, fijo) {
    ...
    // El contenido sale del catálogo estático `TOOLTIPS` (texto del código, sin
    // datos de usuario ni del servidor), así que `innerHTML` no expone XSS. Si
    // alguna vez se interpola un dato dinámico, hay que escaparlo antes.
    tooltip.innerHTML = `
