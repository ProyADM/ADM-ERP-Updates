...
# ============================================================
# SESIONES EN ARCHIVO: CARPETA EXPLICITA, ABSOLUTA Y PROPIA
# ============================================================
# `flask_session/defaults.py` resuelve el default como
# `os.path.join(os.getcwd(), 'flask_session')`: o sea que la carpeta de sesiones
# quedaba dependiendo del directorio de trabajo CON EL QUE SE ARRANCO la app, no
# del directorio donde vive la app. La app se arranca por varios caminos (el
# .vbs del instalador, iniciar.bat, `python app.py` a mano), no todos con el
# mismo CWD, asi que eso era fragilidad pura: la carpeta de sesiones tiene que
# ser SIEMPRE `{app}\flask_session`.
#
# Se fija ANTES de `Session(app)` porque cachelib crea la carpeta en el
# `__init__` del backend (cachelib/file.py:78), leyendo `SESSION_FILE_DIR` en ese
# momento. Por eso tambien se asegura la carpeta aca (mismo `__init__` la crea,
# pero dejarlo explicito deja la garantia en el arranque y no en un detalle
# interno de la libreria).
SESSION_FILE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                'flask_session')
# `SIDESYS_SESSION_FILE_DIR` redirige la carpeta de sesiones. Es el mismo espiritu
# que `SIDESYS_INSTALL_DIR` (que usa el E2E para no tocar la instalacion real):
# sirve para correr la app en un sandbox y para poder MEDIR la guardia de abajo
# (en el checkout, `<raiz>\flask_session` tiene el atributo R de OneDrive y ahi
# `os.rmdir` da WinError 5: la carpeta no se puede hacer desaparecer, que es
# justamente lo que la guardia tiene que reparar). Sin la variable, la carpeta es
# SIEMPRE la del directorio de `app.py`.
_override_sesiones = os.environ.get('SIDESYS_SESSION_FILE_DIR')
if _override_sesiones:
    SESSION_FILE_DIR = os.path.abspath(_override_sesiones)
    print(f'📂 Carpeta de sesiones (SIDESYS_SESSION_FILE_DIR): {SESSION_FILE_DIR}')
app.config['SESSION_FILE_DIR'] = SESSION_FILE_DIR
try:
    os.makedirs(SESSION_FILE_DIR, exist_ok=True)
except OSError as e:
    # No se aborta el arranque por esto: la guardia por request de abajo
    # reintenta crear la carpeta en cada request que la necesite.
    app.logger.warning(
        f'⚠️ no se pudo crear la carpeta de sesiones {SESSION_FILE_DIR}: {e}')

Session(app)


@app.before_request
def _recrear_carpeta_de_sesiones():
    """Recrea la carpeta de sesiones si desaparecio (red de seguridad).

    El arreglo de fondo es no borrarla nunca con la app arriba (ver
    `modules/shared/relanzar_app.py`: la limpieza de `flask_session` ocurre solo
    ANTES de arrancar). Esto es la red: si algo externo la borra igual (el
    incidente de 1.1.3, un antivirus, un `rmtree` de por medio), la app se cura
    SOLA en el request siguiente en vez de quedar sin poder iniciar sesion hasta
    el proximo arranque — que fue el sintoma real: `POST /api/auth/login`
    respondia 200 y el `GET /api/auth/current_user` inmediato respondia 401, en
    bucle, con un `FileNotFoundError` por request.

    Es un `os.path.isdir` por request (barato). El aviso es un `logger.warning`
    a proposito VISIBLE: un borrado externo no puede pasar en silencio.
    """
    if not os.path.isdir(SESSION_FILE_DIR):
        try:
            os.makedirs(SESSION_FILE_DIR, exist_ok=True)
            app.logger.warning(
                f'⚠️ la carpeta de sesiones {SESSION_FILE_DIR} no existia: '
                f'recreada. Un borrado externo dejo la app sin poder guardar '
                f'sesiones (revisar el relanzador/antivirus).')
        except OSError as e:
            app.logger.error(
                f'⚠️ no se pudo recrear la carpeta de sesiones '
                f'{SESSION_FILE_DIR}: {e}')
    return None
