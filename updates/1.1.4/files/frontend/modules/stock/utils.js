...
// ============================================================
// CLAVE DE IDEMPOTENCIA (un envio = un POST con su clave)
// ============================================================

// Texto del 409 por clave reutilizada: la clave guardada quedo pegada a OTRO
// contenido, asi que no sirve para reintentar este envio. Una sola fuente para
// los tres archivos de envios (ajustes, transferencias y excel).
const MENSAJE_CLAVE_REUTILIZADA = 'El envío anterior ya había quedado registrado con otro contenido, así que su clave no sirve para este envío. Se descartó la clave: volvé a enviar y se generará una nueva.';

// Clave de idempotencia: una por envio (se reusa solo si hay que reintentar el mismo POST).
function nuevaClaveIdempotencia() {
  if (window.crypto && typeof window.crypto.randomUUID === 'function') return window.crypto.randomUUID();
  return 'k-' + Date.now() + '-' + Math.random().toString(36).slice(2, 12);
}

// Claves pendientes por envio. El estado vive en window porque estos archivos
// son scripts clasicos que comparten el ambito global (un `const` repetido en
// dos archivos romperia el segundo script).
function clavesDeEnvioStock() {
  if (!window.__clavesIdempotenciaStock) window.__clavesIdempotenciaStock = {};
  return window.__clavesIdempotenciaStock;
}

// La clave del envio: se crea la primera vez y se REUSA mientras el envio no
// haya terminado bien (asi un reintento no duplica el movimiento).
function claveDeEnvio(envio) {
  const claves = clavesDeEnvioStock();
  if (!claves[envio]) claves[envio] = nuevaClaveIdempotencia();
  return claves[envio];
}

// Un envio exitoso se cierra: el proximo POST de ese envio arranca con clave nueva.
function cerrarEnvio(envio, ok) {
  if (ok) delete clavesDeEnvioStock()[envio];
}

// La clave guardada dejo de servir (p.ej. el 409 STK_VAL_CLAVE_REUTILIZADA: ya
// quedo registrada con OTRO contenido). Se olvida para que el proximo envio de
// ese formulario genere una clave nueva y el usuario pueda reintentar.
function olvidarEnvio(envio) {
  delete clavesDeEnvioStock()[envio];
}
...
window.nuevaClaveIdempotencia = nuevaClaveIdempotencia;
window.claveDeEnvio = claveDeEnvio;
window.cerrarEnvio = cerrarEnvio;
window.olvidarEnvio = olvidarEnvio;
