...
def guardar_estado_actualizacion(estado, mensaje='', version=None, error=None):
    """Escribe el estado real del proceso de actualización en disco.
    Esto es lo que el frontend debe consultar en vez de simular con un timer."""
    try:
        os.makedirs(os.path.dirname(UPDATE_STATUS_FILE), exist_ok=True)
        with open(UPDATE_STATUS_FILE, 'w', encoding='utf-8') as f:
            json.dump({
                'estado': estado,  # 'descargando' | 'instalando' | 'completado' | 'error'
                'mensaje': mensaje,
                'version': version,
                'error': error,
                'timestamp': time.time()
...
def descargar_archivos_diferenciales(version_info):
    ...
    try:
        ...
        # 🔴 NUEVO: obtener la lista de archivos que el servidor marcó como eliminados
        # en esta versión, para no dejarlos huérfanos en el disco del cliente.
        archivos_eliminar = []
        if deleted_url:
            try:
        ...
        for file_path, info_remoto in manifest_remoto.items():
            ...
            if os.path.exists(local_path):
                with open(local_path, 'rb') as f:
                    # 🔴 CORRECCIÓN: generar_update.py firma el manifest remoto con SHA-256,
                    # antes se comparaba contra MD5 y jamás coincidía -> se re-descargaba TODO siempre.
        ...
        for idx, file_path in enumerate(archivos_actualizar, start=1):
            # 🔴 CORRECCIÓN: normalizar la ruta para URL y evitar '//' (files_url ya termina en '/')
...
def descargar_actualizacion_completa(version_info):
    """Fallback: descarga la actualización completa (solo si el server publica un ZIP;
    generar_update.py ya no lo hace por defecto, esto queda como red de seguridad)"""
    try:
        import requests
        import zipfile
...
def instalar_actualizacion_diferencial(archivos_eliminar=None):
    ...
    y borra los que el servidor marcó como eliminados en esta versión."""
    try:
        ...
        # 🔴 NUEVO: borrar del disco del cliente los archivos que ya no existen
        # en la versión nueva, para que no queden archivos viejos/huérfanos duplicando espacio.
        archivos_borrados = 0
...
def verificar_y_actualizar():
    ...
    # ✅ Solo acá, con los archivos ya copiados y version.txt ya escrito, se marca 'completado'.
    # El frontend debe esperar este estado antes de matar el proceso y reiniciar.
...
def api_update_install():
    ...
    # Limpiar estado previo para que el polling del frontend no lea un 'completado' viejo
...
def api_update_status():
    """
    Devuelve el estado REAL del proceso de actualización en curso.
    El frontend debe pollear esto en vez de simular tiempos con un timer,
    y recién llamar a /api/reiniciar cuando estado == 'completado'.
    """
