...
# ============================================================
# RUTAS DE COTIZACIONES - SIDESYS ERP (VERSIÓN CORREGIDA)
# ============================================================
...
def guardar():
    ...
    try:
        ...
        # ✅ CORREGIDO: Validar que sea una lista
        ...
        for item in data:
            ...
            if not item.get("fecha"):
                errores.append("Fecha no proporcionada")
                continue
                
            if not validar_fecha(item["fecha"]):
                errores.append(f"Fecha inválida: {item['fecha']}")
                continue
                
            try:
                cotizacion = float(item["cotizacion"])
                if cotizacion <= 0:
                    errores.append(f"Cotización inválida (debe ser positiva): {cotizacion}")
                    continue
            except (ValueError, TypeError):
                errores.append(f"Cotización inválida: {item.get('cotizacion')}")
                continue

            bases = item.get("bases", PAISES_PERMITIDOS)
            
            # ✅ CORREGIDO: Validar que las bases sean válidas
            bases_validas = [b for b in bases if b in PAISES_PERMITIDOS]
            if not bases_validas:
                errores.append(f"No hay bases válidas para: {bases}")
                continue
            ...
            for sigla in bases_validas:
                ...
                try:
                    ...
                    cursor.execute(query, (
                        ...
                        cotizacion,
                        'DL', 'PS', item["fecha"],
                        'DL', 'PS', item["fecha"], cotizacion
                    ))
                    conn.commit()
                    ok += 1
                    
                except pyodbc.Error as e:
                    error_msg = f"{sigla} {item['fecha']}: {str(e)}"
        ...
        if bases_con_error:
            return jsonify({
                ...
                "mensaje": f"⚠️ No se pudo guardar en: {', '.join(set(bases_con_error))}."
            })
...
def importar_excel():
    ...
    try:
        ...
        items = []
        try:
            # ✅ CORREGIDO: Validar que sea un Excel válido
            try:
            ...
            for sigla in item["bases"]:
                ...
                try:
                    ...
                    cursor.execute(query, (
                        ...
                        item["cotizacion"],
                        'DL', 'PS', item["fecha"],
                        'DL', 'PS', item["fecha"], item["cotizacion"]
                    ))
                    conn.commit()
                    ok += 1
                    
                except pyodbc.Error as e:
                    errores.append(f"{sigla} {item['fecha']}: {str(e)}")
                    bases_con_error.append(sigla)
                    logger.error(f"Error SQL en {sigla}: {e}")
                except Exception as e:
                    errores.append(f"{sigla} {item['fecha']}: {str(e)}")
        ...
        if bases_con_error:
            return jsonify({
                ...
                "errores": errores,
                "bases_con_error": list(set(bases_con_error)),
                "mensaje": f"⚠️ No se pudo importar en: {', '.join(set(bases_con_error))}."
            })
...
def cotizacion():
    ...
    try:
        ...
        # ✅ CORREGIDO: Validar fecha (si se proporciona)
        ...
        # Usar el primer país disponible para consultar la cotización
        primer_pais = PAISES_PERMITIDOS[0] if PAISES_PERMITIDOS else None
        if not primer_pais:
            return jsonify({"cotizacion": 1.0})

        conn = None
        try:
            conn = obtener_conexion_pais_segura(primer_pais)
            cursor = conn.cursor()
            ...
            if row and row[0]:
                try:
                    return jsonify({"cotizacion": float(str(row[0]).replace(",", "."))})
                except (ValueError, TypeError):
                    return jsonify({"cotizacion": 1.0})
                    
        except pyodbc.Error as e:
            logger.error(f"Error SQL en cotización: {e}")
        except Exception as e:
            logger.error(f"Error inesperado en cotización: {e}")
        finally:
            if conn:
                try:
                    conn.close()
