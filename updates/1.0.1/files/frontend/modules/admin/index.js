...
function adminMulti(nombre, items, seleccionados, opciones) {
  ...
  if (opciones.comodin) {
    ...
    html += '<label class="admin-check admin-check-todos"><input type="checkbox"' +
            ' data-admin-todos="' + escapeHTML(nombre) + '"' +
            (marcadoComodin ? ' checked' : '') + '> ' +
            escapeHTML(opciones.comodin) + '</label>';
  }
  ...
  items.forEach(function (item) {
    ...
    // `verificado === false` = el backend no verifica este permiso hoy: se pinta
    // en gris con la insignia, para no documentar como real algo que no controla.
    ...
    // El identificador crudo va debajo: es lo que el admin necesita para
    // cruzarlo con la API y con los logs.
    var pie = [];
    if (descripcion) pie.push(escapeHTML(descripcion));
    if (sinEfecto) pie.push('<b>sin efecto todavía</b>');
    pie.push('<code>' + escapeHTML(valor) + '</code>');
    html += '<label class="admin-check' + (sinEfecto ? ' admin-check-gris' : '') + '"' +
            ...
            '" value="' + escapeHTML(valor) + '"' + tildado + '>' +
            '<span class="admin-check-texto">' + escapeHTML(etiqueta) +
            '<span class="admin-check-pie">' + pie.join(' · ') + '</span></span></label>';
  });
  return html;
}
