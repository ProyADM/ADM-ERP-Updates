...
async function buscarConsolidado(actualizarMarcas = false) {
  ...
  const marcasAntes = window._marcasAntesDeToast || null;
  window._marcasAntesDeToast = null;
  let basesConNuevos = new Set();

  if (marcasAntes) {
    try {
      const desde = JSON.stringify(marcasAntes);
      const [rMarcas, rMovidos] = await Promise.all([
        fetch(`${API}/stock/consolidado/marcas?desde=${encodeURIComponent(desde)}`).then(r => r.json()),
        fetch(`${API}/stock/consolidado/movidos?desde=${encodeURIComponent(desde)}`).then(r => r.json()),
      ]);
      for (const [base, info] of Object.entries(rMarcas)) {
        if (info.nuevos > 0) {
          const sigla = (BASES_DISPONIBLES_FRONT[base] || {}).sigla || base;
          basesConNuevos.add(sigla);
          if (base === "plataforma_ur") { basesConNuevos.add("ZF"); basesConNuevos.add("FABRICA"); basesConNuevos.add("TRANSITO"); }
        }
      }
      window._articulosMovidosEnCero = Array.isArray(rMovidos) ? rMovidos : [];
    } catch(e) { window._articulosMovidosEnCero = []; }
  } else {
  ...
  const tbody = filas.map(f => {
    ...
    const tieneNuevo = basesConNuevos.size > 0 && todasCols.some(c => (f[c] || 0) > 0 && basesConNuevos.has(c));
    ...
    const nuevoIndicador = tieneNuevo && f.primera_fila ? ` <span style="font-size:10px;background:#2d7a2d;color:#fff;border-radius:3px;padding:1px 5px;vertical-align:middle;font-weight:600">nuevo</span>` : "";
  ...
  const codsEnTabla = new Set(filas.filter(f => f.codigo).map(f => f.codigo));
  const movEnCero = (window._articulosMovidosEnCero || []).filter(m => !codsEnTabla.has(m.codigo));
  const bgCero = "#fef3f3";
  const tbodyCero = movEnCero.map(m => {
    const codigo = sanitizarValor(m.codigo);
    const nombre = sanitizarValor(m.nombre);
    const basesTexto = Object.keys(m.bases).map(b => sanitizarValor(b)).join(", ");
    const celdasCero = todasCols.map(c => `<td class="${colsUY.includes(c) ? "col-uy" : "col-pais"}"><span style='color:#ccc'>—</span></td>`).join("");
    return `<tr style="background:${bgCero}">
      <td style="${stickyBase}left:0;background:${bgCero};font-weight:700;color:var(--primary)">${codigo} <span style="font-size:10px;background:#c0392b;color:#fff;border-radius:3px;padding:1px 5px;vertical-align:middle;font-weight:600">vacío</span></td>
      <td style="${stickyBase}left:80px;background:${bgCero};color:var(--ink-soft);font-style:italic">${nombre} <span style="font-size:11px;opacity:0.7">(${basesTexto})</span></td>
      ${celdasCero}
      <td class="col-total"><span style='color:#ccc'>—</span></td>
    </tr>`;
  }).join("");
  ...
  const vaciasTexto = movEnCero.length > 0 ? ` · <span style="color:#c0392b">● ${movEnCero.length} bajaron a 0</span>` : "";
...
function irConsolidadoDesdeToast() {
  window._marcasAntesDeToast = _marcasConsolidadoGlobal ? { ..._marcasConsolidadoGlobal } : null;
  cerrarToast();
...
async function _actualizarMarcasConsolidado() {
  try {
    const r = await fetch(`${API}/stock/consolidado/marcas`);
    const data = await r.json();
    _marcasConsolidadoGlobal = {};
    for (const [base, info] of Object.entries(data)) {
      _marcasConsolidadoGlobal[base] = typeof info === 'object' ? info.max_mov : info;
    }
  } catch (e) { /* silencioso */ }
}

async function _checkCambiosConsolidado() {
  if (!_marcasConsolidadoGlobal || _toastVisibleGlobal) return;
  try {
    const desde = JSON.stringify(_marcasConsolidadoGlobal);
    const r = await fetch(`${API}/stock/consolidado/marcas?desde=${encodeURIComponent(desde)}`);
    const nuevas = await r.json();
    const basesConCambios = Object.entries(nuevas).filter(([b, info]) => info.nuevos > 0);
    if (basesConCambios.length === 0) return;

    const seccionActiva = document.querySelector(".section.active");
    const enConsolidado = seccionActiva && seccionActiva.id === "stock-consolidado";

    if (enConsolidado) {
      const consBanner = document.getElementById("cons-banner");
      if (consBanner) consBanner.style.display = "flex";
    } else {
      const detalle = basesConCambios.map(([b, info]) => {
        const sigla = (BASES_DISPONIBLES_FRONT[b] || {}).sigla || b;
        return `${sigla}: ${info.nuevos}`;
      }).join(" · ");
      _toastVisibleGlobal = true;
      const toastBody = document.getElementById("stock-toast-body");
      const toastPaises = document.getElementById("stock-toast-paises");
      const toast = document.getElementById("stock-toast");
      if (toastBody) toastBody.textContent = `${basesConCambios.length} país${basesConCambios.length === 1 ? "" : "es"} con movimientos nuevos`;
      if (toastPaises) toastPaises.textContent = detalle;
...
console.log('✅ Stock - Consolidado cargado (XSS sanitizado)');