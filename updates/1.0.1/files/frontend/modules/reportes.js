// js/reportes.js - CORREGIDO (XSS SANITIZADO)

let datosReporte = [];
let filtroAnio = '';
let filtroMes = '';
let monedaActual = 'PS';
let reporteActual = 'contratos';
...
function sanitizarCSV(str) {
    ...
    const sanitized = String(str).replace(/[=+\-@\t\r\n]/g, ' ');
    return sanitized;
}
...
function cargarReporte() {
    ...
    if (seccionActiva) {
        seccionActiva.style.display = 'none';
    }
    
    document.querySelectorAll('.section').forEach(s => s.classList.remove('active'));
    
    monedaActual = 'PS';
    reporteActual = 'contratos';
    ...
    fetch(`/api/reportes/anos?base=${base}&sociedad=${sociedad}`)
        ...
        .then(data => {
            ...
            if (data.success && data.anos && data.anos.length > 0) {
                ...
                if (anosLimitados.includes(añoActual)) {
                    select.value = añoActual;
                } else {
                    select.value = anosLimitados[0] || '';
                }
                filtroAnio = select.value;
                console.log('✅ Años cargados, seleccionado:', filtroAnio);
                
                setTimeout(() => {
                    if (typeof ejecutarReporte === 'function') {
                        ejecutarReporte();
                    }
                }, 300);
            } else {
                ...
                if (typeof toastWarning === 'function') {
                    toastWarning('No se encontraron años disponibles', 'Reportes');
                }
            }
        })
        .catch(error => {
            ...
            if (typeof toastError === 'function') {
                toastError('Error al cargar años: ' + escapeHTML(error.message), 'Reportes');
            }
        });
}
...
function ejecutarReporte() {
    ...
    .then(data => {
        ...
        console.log('📊 data.resumen:', data.resumen);
        ...
        if (data.success) {
            ...
            if (data.total_registros > 0) {
                if (typeof toastSuccess === 'function') {
                    toastSuccess(`✅ ${data.total_registros} registros encontrados`, 'Reportes');
                }
            } else {
                if (typeof toastWarning === 'function') {
                    toastWarning('⚠️ No se encontraron registros para los filtros seleccionados', 'Reportes');
                }
            }
        } else {
            console.error('❌ data.success es false:', data.error);
            if (typeof toastError === 'function') {
                toastError('❌ Error: ' + escapeHTML(data.error || 'Error desconocido'), 'Reportes');
            }
    ...
    .catch(error => {
        ...
        mostrarLoading(false);
        if (typeof toastError === 'function') {
            toastError('❌ Error al cargar el reporte: ' + escapeHTML(error.message), 'Reportes');
        }
        renderizarTablaDinamica([], 'PS');
        renderizarResumenPorMoneda([]);
        renderizarResumenAnualCompleto([]);
...
function renderizarResumenAnualCompleto(datos) {
    ...
    datosPS.forEach(row => {
        ...
        if (!gruposPS[key]) {
            gruposPS[key] = { ANIO: row.ANIO, MES: row.MES, IMPORTE: 0 };
        }
    ...
    datosDL.forEach(row => {
        const key = `${row.ANIO}-${String(row.MES).padStart(2, '0')}`;
        if (!gruposDL[key]) {
            gruposDL[key] = { ANIO: row.ANIO, MES: row.MES, IMPORTE: 0 };
        }
    ...
    const resumen = Array.from(allKeys).map(key => {
        ...
        return {
            ANIO: parseInt(anio),
            MES: parseInt(mes),
            PS_IMPORTE: ps.IMPORTE,
            DL_IMPORTE: dl.IMPORTE
        };
    }).sort((a, b) => {
    ...
    let totalPSGeneral = 0;
    let totalDLGeneral = 0;
    ...
    resumen.forEach(row => {
        ...
        const psFormateado = formatearNumero(row.PS_IMPORTE);
        const dlFormateado = formatearNumero(row.DL_IMPORTE);
        html += `
            <tr>
                ...
                <td style="padding:6px 12px; border-bottom:1px solid #f1f5f9; text-align:right; color:#2563eb;">$${psFormateado}</td>
                <td style="padding:6px 12px; border-bottom:1px solid #f1f5f9; text-align:right; color:#16a34a;">$${dlFormateado}</td>
            </tr>
        `;
    });
...
function renderizarTablaDinamica(datos, moneda) {
    console.log('📊 renderizarTablaDinamica() llamado');
    ...
    const keysOrdenadas = keys.sort((a, b) => {
        const gA = grupos[a];
        const gB = grupos[b];
        if (gA.CLIENTE !== gB.CLIENTE) {
            return gA.CLIENTE.localeCompare(gB.CLIENTE);
        }
...
function mostrarLoading(activo) {
    ...
    if (loading) {
        loading.style.display = activo ? 'block' : 'none';
    }
}
...
function exportarExcel() {
    if (datosReporte.length === 0) {
        if (typeof toastWarning === 'function') {
            toastWarning('⚠️ No hay datos para exportar', 'Reportes');
        }
        return;
    }
    ...
    if (datosExportar.length === 0) {
        if (typeof toastWarning === 'function') {
            toastWarning(`⚠️ No hay datos para exportar con los filtros actuales`, 'Reportes');
        }
        return;
    }
    ...
    datosExportar.forEach(row => {
        csv += columnas.map(col => {
            let valor = row[col] || '';
            // ✅ CORREGIDO: Sanitizar CSV para prevenir inyección
            if (typeof valor === 'string') {
                valor = sanitizarCSV(valor);
                // Si contiene comas o comillas, envolver en comillas
    ...
    if (typeof toastSuccess === 'function') {
        toastSuccess('✅ Reporte exportado', 'Reportes');
    }
}

// ============================================================
// FUNCIÓN PARA CAMBIAR ENTRE REPORTES
// ============================================================
    ...
    if (container) {
        ...
        switch(tipo) {
            case 'pendiente-cobro':
                if (typeof window.cargarPendienteCobro === 'function') {
                    window.cargarPendienteCobro();
                }
                break;
            case 'consolidado-pais':
                import('/frontend/modules/reportes/consolidado_pais.js')
                .then(module => {
                    if (module.cargarConsolidadoPais) {
                        module.cargarConsolidadoPais();
                    } else {
                        console.warn('⚠️ cargarConsolidadoPais no está disponible');
                    }
                })
                .catch(e => {
                    console.error('❌ Error cargando consolidado_pais:', e);
                });
            break;
            case 'consolidado-total':
                if (typeof window.cargarConsolidadoTotal === 'function') {
                    window.cargarConsolidadoTotal();
                }
                break;
            case 'contratos':
                cargarAnos();
...
window.cargarAnos = cargarAnos;
window.escapeHTML = escapeHTML;
window.sanitizarCSV = sanitizarCSV;

console.log('✅ reportes.js cargado correctamente (XSS sanitizado)');