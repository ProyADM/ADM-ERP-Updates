...
// ============================================================
// FUNCIONES DE SANITIZACIÓN (NUEVO)
// ============================================================
...
function cambiarModulo(modulo) {
  ...
  // ✅ CORREGIDO: Siempre cargar el template correspondiente
...
// ✅ NUEVO: Cargar módulo CXP al iniciar
function cargarCXPInicial() {
  console.log('📦 Cargando módulo CXP inicial...');
  // Verificar si el template CXP ya está cargado
...
async function inicializarBase(baseForzada, sociedadForzada) {
  ...
  if (multibaseChecklist) {
    ...
    multibaseChecklist.innerHTML = basesCache.map(b => {
      ...
      return `<label style="display:flex;align-items:center;gap:8px;text-transform:none;font-weight:500;margin-top:6px">
        <input type="checkbox" class="multibase-check" value="${codigo}" style="width:auto"> ${label}
      </label>`;
    }).join("");
  }
  ...
  if (depMultibaseChecklist) {
    ...
    depMultibaseChecklist.innerHTML = basesCache.map(b => {
      ...
      const label = escapeHTML(b.label);
      return `<label style="display:flex;align-items:center;gap:8px;text-transform:none;font-weight:500;margin-top:6px">
        <input type="checkbox" class="dep-multibase-check" value="${codigo}" style="width:auto"> ${label}
      </label>`;
    }).join("");
  }
...
function actualizarIndicadorBase() {
  ...
  if (baseLabel) baseLabel.textContent = baseInfo.label;
...
async function seleccionarBase(codigo) {
  ...
  if (moduloActual === 'reportes' && typeof ejecutarReporte === 'function') {
    ejecutarReporte();
  }
...
function verificarNotificacion() {
    ...
    // Obtener la versión instalada desde localStorage
    ...
    fetch('/api/update/notification')
        ...
        .then(data => {
            if (data.has_update) {
                ...
                // Comparar versiones: solo mostrar si la remota es mayor que la instalada
...
function instalarActualizacion() {
    ...
    .then(data => {
        if (data.success) {
            actualizarProgreso(50, 'Descargando archivos...');
            ...
            const intervalo = setInterval(() => {
                ...
                if (progreso >= 90) {
                    ...
                    actualizarProgreso(95, 'Instalando...');
                } else {
                    actualizarProgreso(progreso, `Descargando... ${progreso}%`);
                }
            }, 500);
            ...
            setTimeout(() => {
                ...
                // ✅ Guardar la versión instalada en localStorage
                const nuevaVersion = data.version || '1.0.1';
...
// ============================================================
// 🆕 OBTENER DATOS DEL USUARIO ACTUAL (NUEVO)
// ============================================================
...
document.addEventListener('DOMContentLoaded', function() {
    // ✅ Cargar usuario automáticamente al inicio
