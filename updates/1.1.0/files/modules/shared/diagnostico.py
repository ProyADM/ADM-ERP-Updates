...
# Numerador del ERP para las partidas: `SIST_NUSI.NUSI_NUMERADOR_ID = 6`
# (`NUSI_NOMBRE = 'Ultimo número de partida'`, semántica "último usado"). Existe
# en las 11 bases, medido en solo lectura con `_investigacion_gt/166_*` y `169_*`
# (`docs/INCIDENTE_PARTIDAS_AR_2026-09-21.md` §1).
#
# Fuentes de que el 6 es el ÚNICO numerador de partidas que hay que tocar: el
# controlador lo verificó al escribir el brief ("en ninguna hay otra
# tabla/numerador de partidas") y la revisión de `cb72f39` lo midió con grep
# sobre el árbol: una sola aparición de `INSERT INTO STOC_PART` en el código de
# producción (la de `_movimiento_partidas`) y los ids de `SIST_NUSI` que usa el
# código son 5, 4, 23 y 6 (`review-cb72f39.md` §6).
NUMERADOR_PARTIDAS_ID = 6

# Chequeo del numerador de partidas: el contador del ERP contra el máximo real
# de `STOC_PART` en esa base. Es un `SELECT`: no escribe (toma locks de lectura
# como cualquier lectura y puede quedar esperando a un escritor).
SQL_NUMERADOR_PARTIDAS = (
    'SELECT n.NUSI_ULTIMO_NUMERO AS contador, '
    'ISNULL((SELECT MAX(PART_PARTIDA) FROM STOC_PART), 0) AS maximo '
    'FROM SIST_NUSI n WHERE n.NUSI_NUMERADOR_ID = ' + str(NUMERADOR_PARTIDAS_ID)
)
...
def _probar_base(base, espera):
    ...
    try:
        ...
        # Chequeo de SOLO LECTURA del numerador de partidas, en la conexión que
        # ya está abierta: si el contador del ERP quedó atrás, su próxima alta de
        # partida choca con `PART_PK1` (2627, el incidente del 21/09/2026). Si
        # ESTA consulta falla, la base no pasa a "caída": se informa el motivo.
        try:
            resultado['partidas'] = estado_numerador_partidas(cursor)
        except Exception as e:
            resultado['partidas'] = {'contador': None, 'maximo': None,
                                     'aviso': 'no se pudo leer el numerador de partidas: %s' % str(e)[:200]}
    except Exception as e:
...
def aviso_numerador_partidas(contador, maximo):
    """Aviso (texto) si el numerador de partidas quedó ATRÁS del máximo real.

    `contador` es `SIST_NUSI.NUSI_ULTIMO_NUMERO` del numerador 6 y `maximo` es
    `MAX(PART_PARTIDA)` de esa base, los dos leídos por `estado_numerador_partidas`.

    Con el contador atrás, la próxima alta de partida del ERP pide un número ya
    usado y choca con la clave primaria `PART_PK1` (error 2627 `PART_RK1`): es el
    incidente del 21/09/2026 en la base AR, medido con contador 1486 y
    `MAX(PART_PARTIDA)` 1487 (`docs/INCIDENTE_PARTIDAS_AR_2026-09-21.md` §1 y §3).
    Contador al día o adelantado no es un problema (en `plataforma_ur` el contador
    está 3 adelante, §3).

    Devuelve None cuando no hay nada que avisar. Si no se pudieron leer los dos
    valores devuelve un texto que NO afirma desfasaje: informa que no se pudo
    leer, que no es lo mismo que estar al día (defensivo: sin poder medir, no se
    declara un desfasaje).
    """
    if contador is None or maximo is None:
        return ('no se pudo leer el numerador de partidas (SIST_NUSI id '
                + str(NUMERADOR_PARTIDAS_ID) + ') en esta base')
    contador, maximo = int(contador), int(maximo)
    if contador >= maximo:
        return None
    return ('el numerador de partidas (SIST_NUSI id ' + str(NUMERADOR_PARTIDAS_ID)
            + ') quedó en %d y STOC_PART ya tiene hasta %d: la próxima alta de partida del ERP '
              'choca con PART_PK1 (error 2627).' % (contador, maximo))


def estado_numerador_partidas(cursor):
    """(SOLO LECTURA) `{contador, maximo, aviso}` del numerador de partidas.

    Recibe un cursor ya conectado a la base que se está probando (es el mismo que
    hizo el `SELECT 1`): no abre conexiones ni escribe nada.
    """
    cursor.execute(SQL_NUMERADOR_PARTIDAS)
    fila = cursor.fetchone()
    if fila is None:
        # Sin la fila del numerador 6 no hay desfasaje que medir: se informa la
        # falta, que además es el fail-closed del alta de partidas.
        return {'contador': None, 'maximo': None,
                'aviso': 'falta el numerador de partidas (SIST_NUSI id '
                         + str(NUMERADOR_PARTIDAS_ID) + ') en esta base'}
    contador, maximo = fila[0], fila[1]
    # `int(...)` defensivo. El controlador midió los tipos de la columna con el
    # probe `_investigacion_gt/181_tipo_nusi_ultimo_numero.py` (solo lectura, 11
    # bases): `SIST_NUSI.NUSI_ULTIMO_NUMERO` es int(10,0) en las once, igual que
    # `NUSI_NUMERADOR_ID`, así que hoy el riesgo no existe. El cast queda por el
    # precedente del repo: un DECIMAL no lo serializa `jsonify` y se cae el
    # endpoint entero del panel (`modules/cxp/routes.py:417-420`). El None se
    # conserva: `aviso_numerador_partidas` sabe que "no se pudo leer" no es un
    # desfasaje.
    contador = None if contador is None else int(contador)
    maximo = None if maximo is None else int(maximo)
    return {'contador': contador, 'maximo': maximo,
            'aviso': aviso_numerador_partidas(contador, maximo)}
...
def estado_canal_actualizaciones():
    ...
    abrir archivos a mano. Incluye también `ultimo_estado` (lo ultimo que dejo el
    updater en `update_status.json`), para responder "¿cómo se aplicó la última?
    ¿falló?".
    """
    try:
    ...
    # Ultimo estado del updater (`update_status.json`): con esto la fila
    # "Versión" del panel dice COMO y CUANDO se aplico la ultima actualizacion
    # (automatica al inicio o manual) y si esa ultima vez fallo.
    ultimo_estado = {}
    try:
        from app import leer_estado_actualizacion
        e = leer_estado_actualizacion() or {}
        ultimo_estado = {k: e.get(k) for k in
                         ('estado', 'version', 'origen', 'aplicada_en', 'error', 'intentos')
...
def informe(bases=None, horas=24):
    ...
    # Bases con ALGÚN aviso del numerador de partidas. Cuenta cualquier aviso
    # (contador atrasado, fila 6 ausente o "no se pudo leer"), no sólo los
    # desfasajes: es un contador para el informe, y el detalle va por base.
    avisos_partidas = [r for r in informe_sql if (r.get('partidas') or {}).get('aviso')]
    ...
    return {
        ...
        'sql': {
            ...
            'caidas': len(caidas),
            'avisos_partidas': len(avisos_partidas),
        },
