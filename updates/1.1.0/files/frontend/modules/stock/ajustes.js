...
async function hacerAjusteIndividual(signo) {
  ...
  const envio = `ajuste-${prefix}`;
  const resp = await fetch(`${API}/ajuste`, {
    method: "POST",
  ...
  if (resp.ok) {
    ...
    olvidarEnvio(envio);
    mostrarResultadoSeccion(`${prefix}-resultados`, false, MENSAJE_CLAVE_REUTILIZADA);
  } else {
...
async function cargarLote(prefix, signo) {
  ...
  // El lote entero es un envio: una sola clave para todo el comprobante.
  const envio = `ajuste-lote-${prefix}`;
  const resp = await fetch(`${API}/ajuste/lote`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Idempotencia": claveDeEnvio(envio) },
    body: JSON.stringify({ signo, filas })
  });
  const data = await resp.json();
  cerrarEnvio(envio, resp.ok);
  if (resp.ok) {
