...
async function hacerTransferenciaIndividual() {
  ...
  const resp = await fetch(`${API}/transferencia`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Idempotencia": claveDeEnvio(envio) },
    body: JSON.stringify(body)
  });
  const data = await resp.json();
  // El envio se cierra solo con exito: si fallo, el reintento reusa la clave.
  cerrarEnvio(envio, resp.ok);
  if (resp.ok) {
  ...
  } else if (resp.status === 409 && data.codigo === 'STK_VAL_CLAVE_REUTILIZADA') {
    // La clave quedo pegada a OTRO contenido: no sirve para reintentar. Se
    // olvida para que el proximo envio genere una clave nueva.
    olvidarEnvio(envio);
    mostrarResultadoSeccion("trm-resultados", false, MENSAJE_CLAVE_REUTILIZADA);
  } else {
...
async function cargarLoteTransferencia() {
  ...
  // El lote entero es un envio: una sola clave para todas las filas.
  const envio = "transferencia-lote";
  const resp = await fetch(`${API}/transferencia/lote`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Idempotencia": claveDeEnvio(envio) },
    body: JSON.stringify({ filas })
  });
  const data = await resp.json();
  // La clave se cierra solo si el envio salio bien Y el backend confirma que
  // NINGUNA fila fallo (`todas_ok`): con filas fallidas la clave queda guardada
  // y reintentar el mismo lote no vuelve a aplicar las que ya entraron.
  cerrarEnvio(envio, resp.ok && data.todas_ok !== false);
  if (!resp.ok) {
    if (resp.status === 409 && data.codigo === 'STK_VAL_CLAVE_REUTILIZADA') {
      // La clave quedo pegada a OTRO contenido: se olvida para que el proximo
      // envio genere una clave nueva.
      olvidarEnvio(envio);
      mostrarToastFlotante(false, MENSAJE_CLAVE_REUTILIZADA);
    } else {
      mostrarToastFlotante(false, `Error: ${data.error}`);
    }
    return;
  }
