...
export function sincronizarBruto(i) {
    ...
    // El impuesto especial (IDP, turismo) va DENTRO del bruto y en FCC no
    // existe: el total ya es el bruto. Mismo criterio que recalcFact.
    let especial = parseFloat(document.getElementById(`f${i}_especial`)?.value) || 0;
    if (tipo === 'FCC') especial = 0;
    ...
    if (tipo === 'FCC') {
        ...
        const base = Math.round((subtotal / 1.12) * 100) / 100;
        iva = Math.round((subtotal - base) * 100) / 100;
        brutoEsperado = Math.round((base + especial) * 100) / 100;
    }
...
export function agregarRenglon(i, cta='', importe='', cco='', auto=false) {
    const cont = document.getElementById(`f${i}_renglones`);
    if (!cont) return;
    ...
    impInput.step = '0.01';
    // `data-auto="1"` marca el importe que puso la PANTALLA (el bruto del parser):
    // `recalcFact` puede reescribirlo cuando cambia el tipo o el impuesto especial.
    // Al primer tipeo del operador el marcador se limpia y ese valor no se toca mas
    // (antes, un renglon automatico con importe quedaba con el bruto viejo y el
    // alta lo rechazaba por residual al elegir el tipo).
    impInput.dataset.auto = auto ? '1' : '0';
    ...
    impInput.oninput = () => { impInput.dataset.auto = '0'; sincronizarBruto(i); };
